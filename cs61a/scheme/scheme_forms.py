...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        new_signature = signature.first
        lambda_expression = Pair('lambda', Pair(signature.rest, expressions.rest))
        new_expression = Pair(new_signature, Pair(lambda_expression, nil))
        return do_define_form(new_expression, env)
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    lambda_func = LambdaProcedure(formals, expressions.rest, env)
    return lambda_func
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    if is_scheme_false(first_eval):
        return first_eval
    if is_scheme_true(first_eval) and expressions.rest == nil:
        return first_eval
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    first_eval = scheme_eval(expressions.first, env)
    if is_scheme_true(first_eval):
        return first_eval
    if is_scheme_false(first_eval) and expressions.rest == nil:
        return first_eval
    else:
...
def make_let_frame(bindings, env):
    ...
    while current_binding != nil:
        validate_form(current_binding.first, 2, 2)
        names = Pair(current_binding.first.first, names)
        vals = Pair(scheme_eval(current_binding.first.rest.first, env), vals)
