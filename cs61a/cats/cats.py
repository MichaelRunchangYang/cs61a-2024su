...
def pick(paragraphs, select, k):
    ...
    for para in paragraphs:
        if select(para) == True:
            paragraphs_selected.append(para)
    if len(paragraphs_selected) <= k:
        return ''
    else:
        return paragraphs_selected[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def in_para(paragraph):
        low_para_list = split(lower(remove_punctuation(paragraph)))
        for word in low_para_list:
            if word in subject:
                return True
        return False
    return in_para
...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    count = 0
    min_lenth = min(len(typed_words), len(source_words))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if diff_function(typed_word, word, limit) <= limit:
            word_list1[word] = diff_function(typed_word, word, limit)
    if word_list1 == {}:
        return typed_word
    else:
        min_diff = min(word_list1.values())
        return list(word_list1.keys())[list(word_list1.values()).index(min_diff)]
    # END PROBLEM 5
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        times.append([timestamps_per_player[i][x+1] - timestamps_per_player[i][x] for x in range(len(timestamps_per_player[i]) - 1)])
    return words, times
    # END PROBLEM 9
...
def fastest_words(match_object):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    fastest_words_lists = [[] for k in player_indices]
    for i in word_indices:
        j = 0
        for x in player_indices:
            if get_time(match_object, x, i) < get_time(match_object, j, i):
                j = x
        fastest_words_lists[j].append(get_all_words(match_object)[i])
